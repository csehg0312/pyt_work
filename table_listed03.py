...
while True:
    ...
    if event == '_T01_':
        ...
        try:
            kijelolt:list = []*(4*len(os.listdir(os.getcwd())))
            kijelolt = [vals[row] for row in value[event]]
            tmp:str
            tmp = f'{kijelolt[0][1]}{kijelolt[0][0]}'
            ...
            if os.path.isdir(tmP) == True and keyboard.is_pressed('enter'):
                tmP=''
                tmp = ''
            ...
            logging.error(f'{e} has happened', exc_info=True)
    ...
    if event == 'Back':
        ...
        try:
            ...
            logging.error('What happened?', exc_info=True)
