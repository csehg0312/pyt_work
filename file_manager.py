...
def simple_data(ut:str):
    ...
    print(filebase.values())
    print(sys.getsizeof(filebase))
    filebase.update({'Files': deque([x for x in os.listdir(ut) if os.path.isdir(os.path.join(ut,x)) == False])})
    filebase.update({'Size': deque([os.path.join(ut,x) for x in filebase.get('Files')])})
    print(filebase.values())
    return filebase
